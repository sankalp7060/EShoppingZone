    ...
    public class OrderService : IOrderService
    {
        ...
        )
        {
            try
            {
                ...
                // 5. Process wallet payment FIRST (before touching stock)
                ...
                foreach (var item in cart.Items)
                {
                    ...
                    if (!stockUpdated)
                    {
                        // Refund if stock fails after payment
                        await _walletServiceClient.RefundAsync(
                            userId,
                            createdOrder.Id,
            ...
            {
                _logger.LogError(ex, "Order placement failed - potential data inconsistency");
                throw;
            }
        }
