...
using (var scope = app.Services.CreateScope())
{
    ...
    try
    {
        var _ = typeof(EShoppingZone.Order.Infrastructure.Data.ApplicationDbContext).Assembly;
        Console.WriteLine("[DB INFO] Order - using EnsureCreated...");
        await dbContext.Database.EnsureCreatedAsync();
        Console.WriteLine("[DB INFO] Order - schema ready.");
    }
    catch (Exception ex)
    {
        Console.WriteLine($"DB SETUP FAILED (Order): {ex.Message}");
    }
}
